...
echo 'alias xph="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xphchain/core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xphchain/core'"
...
rm -rf "$HOME/.config/@xph"
rm -rf "$HOME/.config/@phantomchain"
rm -rf "$HOME/.config/xph-core"
